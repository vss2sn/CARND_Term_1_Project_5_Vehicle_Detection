...
def process_frame(img):
    ...
    smoothing_window = 20
    draw_img, bbox_list = find_cars(img, svc=svc, X_scaler=X_scaler, orient=9, pix_per_cell=8, cell_per_block=2,spatial_size=(16, 16), hist_bins=16, scale=1.5)
...
def train_test_svm(car_features, notcar_features):
    ...
    t2 = time.time()
    print('Time in seconds to train SVC: ',round(t2 - t, 2))
...
def main():
    ...
    car_features = extract_features(cars, cspace=colorspace, orient=orient,pix_per_cell=pix_per_cell, cell_per_block=cell_per_block,                    hog_channel=hog_channel,spatial_size=spatial_size, hist_bins=hist_bins, hist_range=hist_range)
    notcar_features = extract_features(notcars, cspace=colorspace, orient=orient,pix_per_cell=pix_per_cell, cell_per_block=cell_per_block,hog_channel=hog_channel,spatial_size=spatial_size, hist_bins=hist_bins, hist_range=hist_range)
    # print("Shape = ",len(car_features)) //FOR DEBUG
    svc, X_scaler = train_test_svm(car_features, notcar_features)
    pickle.dump(svc, open( "svc.p", "wb" ) )
    pickle.dump(X_scaler, open( "X_scaler.p", "wb" ) )
    svc = pickle.load(open("svc.p", "rb"))
    X_scaler = pickle.load(open("X_scaler.p", "rb"))
    img = mpi.imread('./test_images/test6.jpg')
    draw_img, bbox_list = find_cars(img, svc=svc, X_scaler=X_scaler, orient=orient,pix_per_cell=pix_per_cell,cell_per_block=cell_per_block,spatial_size=spatial_size, hist_bins=hist_bins, scale=1.5)
    mpi.imsave('draw_img', draw_img)
    images = glob.glob('./test_images/*.jpg')
    for i, img_path in enumerate(images):
        img = mpi.imread(img_path)
        draw_img, bbox_list = find_cars(img, svc=svc, X_scaler=X_scaler, orient=orient,pix_per_cell=pix_per_cell,cell_per_block=cell_per_block,spatial_size=spatial_size, hist_bins=hist_bins, scale=1.5)
        mpi.imsave('./output_images/out_'+str(i), draw_img)
    heat = np.zeros_like(img[:,:,0]).astype(np.float)
    heat = add_heat(heat,bbox_list)
    mpi.imsave('heat', heat)
    heat = apply_threshold(heat,1)
    heatmap = np.clip(heat, 0, 255)
    labels = label(heatmap)
    draw_img = draw_labeled_bboxes(np.copy(img), labels)
    mpi.imsave('Car positions', draw_img)
    mpi.imsave('Heat Map', draw_img)
    return 0
